<template>
  <div>
    <div class="product-header">
      <h1>Báo cáo doanh số theo một ngày</h1>
    </div>
    <div class="block text-center d-flex mb-4">
      <span class="demonstration">Chọn ngày: </span>
      <el-date-picker
        v-model="dataReport.oneDay"
        type="datetime"
...
export default {
  ...
  methods: {
    ...
    searchReport() {
      let oneDay = moment(this.dataReport.oneDay).format("YYYY-MM-DD");
      let data = JSON.stringify({
        oneDay: oneDay,
      });
      getAllRpOneDay(data).then((res) => {
        this.entries = res;
        this.paginateData(this.entries);
      });
      totalOneDay(data).then((res) => {
        this.total = res;
      });
    },
    exportExcel() {
      ...
      exportExcel(oneDay)
        .then((res) => {
          ...
          link.href = url;
          link.setAttribute("download", "bao-cao-" + oneDay + ".xlsx");
    ...
    exportPdf() {
      ...
      exportPdf(oneDay)
        .then((res) => {
          ...
          const link = document.createElement("a");
          link.href = url;
          link.setAttribute("download", "bao-cao-" + oneDay + ".pdf");
          document.body.appendChild(link);
          link.click();
        })
        .catch((err) => {
